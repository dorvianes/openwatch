...
This specification protects the package's current flat wire event contract for the first small package-only hardening slice. It MUST NOT redesign emitted events into an envelope.
...
Future work MAY define `id`, `payload`, `context`, `schema_version`, or `EventFactory`, but this change MUST NOT implement or require them.
